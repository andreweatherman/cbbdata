...
#'
#' @export
cbd_gt_logos <- function(data, team_column, logo_type = 'logo', logo_height = 25) {
  ...
  data <- data %>%
    ...
    mutate(team_logo = glue("<img src='{selected_logo}' style='height: {logo_height}px; width: auto; vertical-align: middle;'> {.data[[as_string(team_column)]]}")) %>%
